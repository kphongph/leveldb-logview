...
logview.config = function(config) {
  ...
  logview.config.batchSize = config.batchSize?config.batchSize:10;
...
var _request = function(config,_rev,callback) {
  var stream = request.get({
    url:config.url,
    qs:{'gt':_rev,'limit':config.batchSize},
    headers: {
